export const balchugTarif = [    
    {
        title: "День",
        label: "Количество мест ограничено",
        categories: [
            {
                button: "1 день",
                price: "1 790 ₽",
                href: "SpaceinWidget.run('ddc6aef0-3f36-4a7a-8d51-4e107502841a');",
                isPay: false,
                list: [
                    { title: "Свободное рабочее место" },
                    { title: "Вода, чай и кофе" },
                    { title: "Принтер и сканер" },
                    { title: "Кухня и душевые" },
                    { title: "Доступ 24/7" },
                ],
                hint: "",
                orderBtnText: "Balchug - День (1 день)"
            },
            // {
            //     button: "10 дней",
            //     price: "15 900 ₽",
            //     href: "SpaceinWidget.run('0ac8ceef-a349-43ea-a902-fc2de129d1b0');",
            //     isPay: false,
            //     list: [
            //         { title: "Свободное рабочее место" },
            //         { title: "Вода, чай и кофе" },
            //         { title: "Принтер и сканер" },
            //         { title: "Кухня и душевые" },
            //         { title: "Доступ 24/7" },
            //     ],
            //     hint: "*Срок действия – 60 дней.",
            //     orderBtnText: "Balchug - День (10 дней)"
            // },
            // {
            //     button: "25 дней",
            //     price: "38 250 ₽",
            //     href: "SpaceinWidget.run('ca9c54d5-ac78-4849-a48d-6490c131f836');",
            //     isPay: false,
            //     list: [
            //         { title: "Свободное рабочее место" },
            //         { title: "Вода, чай и кофе" },
            //         { title: "Принтер и сканер" },
            //         { title: "Кухня и душевые" },
            //         { title: "Доступ 24/7" },
            //     ],
            //     hint: "*Срок действия – 120 дней.",
            //     orderBtnText: "Balchug - День (25 дней)"
            // },
        ]
    },
     {
        title: "25 дней",
        label: "",
        categories: [
            {
                button: "25 дней",
                price: "38 250 ₽",
                href: "SpaceinWidget.run('ca9c54d5-ac78-4849-a48d-6490c131f836');",
                isPay: false,
                list: [
                    { title: "Свободное рабочее место" },
                    { title: "Вода, чай и кофе" },
                    { title: "Принтер и сканер" },
                    { title: "Кухня и душевые" },
                    { title: "Доступ 24/7" },
                ],
                hint: "*Срок действия – 120 дней.",
                orderBtnText: "Balchug - День (25 дней)"
            },
        ]
    },
    // {
    //     title: "Гость",
    //     label: "",
    //     categories: [
    //         {
    //             button: "Неделя",
    //             price: "9 900 ₽",
    //             href: "SpaceinWidget.run('bf2eec79-8926-40b9-8d9d-23a395128825');",
    //             isPay: false,
    //             list: [
    //                 { title: "Свободное рабочее место" },
    //                 { title: "Вода, чай и кофе" },
    //                 { title: "Принтер и сканер" },
    //                 { title: "Кухня и душевые" },
    //                 { title: "Доступ 24/7" },
    //                 { title: "1 час любой переговорной комнаты" },
    //             ],
    //             hint: "",
    //             orderBtnText: "Balchug - Гость (неделя)"
    //         },
    //         {
    //             button: "Месяц",
    //             price: "23 900 ₽",
    //             href: "SpaceinWidget.run('bd4b030c-3522-458b-9a37-6200c4d7c44b');",
    //             isPay: false,
    //             list: [
    //                 { title: "Свободное рабочее место" },
    //                 { title: "Вода, чай и кофе" },
    //                 { title: "Принтер и сканер" },
    //                 { title: "Кухня и душевые" },
    //                 { title: "Доступ 24/7" },
    //                 { title: "5 часов любой переговорной комнаты" },
    //                 { title: "30% скидка на аренду переговорных комнат" },
    //             ],
    //             hint: "",
    //             orderBtnText: "Balchug - Гость (месяц)",
    //         },
    //     ]
    // },
    {
        title: "Резидент",
        label: "",
        categories: [
            {
                button: "1 месяц",
                price: "30 000 ₽",
                href: "SpaceinWidget.run('501e3c77-047e-43db-a8e9-bc7bd4f3fd9a');",
                isPay: false,
                list: [
                    { title: "Фиксированное рабочее место" },
                    { title: "Вода, чай и кофе" },
                    { title: "Принтер и сканер" },
                    { title: "Кухня и душевые" },
                    { title: "Доступ 24/7" },
                    { title: "10 часов любой переговорной комнаты" },
                    { title: "50% скидка на аренду переговорных" },
                ],
                hint: "",
                orderBtnText: "Balchug - Резидент (1 месяц)",
            },
            // {
            //     button: "3 месяца",
            //     price: "85 000 ₽",
            //     href: "SpaceinWidget.run('c5c84909-29c7-4fb4-915b-536241f74f03');",
            //     isPay: false,
            //     list: [
            //         { title: "Фиксированное рабочее место" },
            //         { title: "Вода, чай и кофе" },
            //         { title: "Принтер и сканер" },
            //         { title: "Кухня и душевые" },
            //         { title: "Доступ 24/7" },
            //         { title: "10 часов любой переговорной комнаты" },
            //         { title: "50% скидка на аренду переговорных" },
            //     ],
            //     hint: "",
            //     orderBtnText: "Balchug - Резидент (3 месяца)",
            // },
        ]
    },
];

export const blackTarif = [
    {
        title: "День",
        label: "",
        categories: [
            {
                button: "1 день",
                price: "2 190 ₽",
                href: "SpaceinWidget.run('d54d816a-eb47-481f-ad0d-2feaef9fefa3');",
                isPay: false,
                list: [
                    { title: "Свободное рабочее место" },
                    { title: "Вода, чай и кофе" },
                    { title: "Принтер и сканер" },
                    { title: "Кухня и душевые" },
                    { title: "Доступ 24/7" },
                ],
                hint: "",
                orderBtnText: "Black - День (1 день)"
            },
            // {
            //     button: "10 дней",
            //     price: "19 900 ₽",
            //     href: "SpaceinWidget.run('9e8a1613-4569-48e5-93fb-bb88136f0dc7');",
            //     isPay: false,
            //     list: [
            //         { title: "Свободное рабочее место" },
            //         { title: "Вода, чай и кофе" },
            //         { title: "Принтер и сканер" },
            //         { title: "Кухня и душевые" },
            //         { title: "Доступ 24/7" },
            //     ],
            //     hint: "*Срок действия – 60 дней. Доступно посещение локации Balchug",
            //     orderBtnText: "Black - День (10 дней)"
            // },
            // {
            //     button: "25 дней",
            //     price: "48 250 ₽",
            //     href: "SpaceinWidget.run('0a3e5956-7e70-47ed-89ab-5497a460eab9');",
            //     isPay: false,
            //     list: [
            //         { title: "Свободное рабочее место" },
            //         { title: "Вода, чай и кофе" },
            //         { title: "Принтер и сканер" },
            //         { title: "Кухня и душевые" },
            //         { title: "Доступ 24/7" },
            //     ],
            //     hint: "*Срок действия – 120 дней. Доступно посещение локации Balchug",
            //     orderBtnText: "Black - День (25 дней)"
            // },
        ]
    },
    {
        title: "25 дней",
        label: "",
        categories: [
            {
                button: "25 дней",
                price: "48 250 ₽",
                href: "SpaceinWidget.run('0a3e5956-7e70-47ed-89ab-5497a460eab9');",
                isPay: false,
                list: [
                    { title: "Свободное рабочее место" },
                    { title: "Вода, чай и кофе" },
                    { title: "Принтер и сканер" },
                    { title: "Кухня и душевые" },
                    { title: "Доступ 24/7" },
                ],
                hint: "*Срок действия – 120 дней. Доступно посещение локации Balchug",
                orderBtnText: "Black - День (25 дней)"
            },
        ]
    },
    // {
    //     title: "Гость",
    //     label: "",
    //     categories: [
    //         {
    //             button: "Неделя",
    //             price: "10 900 ₽",
    //             href: "SpaceinWidget.run('c3b5067e-9d55-4e42-a319-18f9123ee57d');",
    //             isPay: false,
    //             list: [
    //                 { title: "Свободное рабочее место" },
    //                 { title: "Вода, чай и кофе" },
    //                 { title: "Принтер и сканер" },
    //                 { title: "Кухня и душевые" },
    //                 { title: "Доступ 24/7" },
    //                 { title: "1 час любой переговорной комнаты" },
    //             ],
    //             hint: "",
    //             orderBtnText: "Black - Гость (неделя)"
    //         },
    //         {
    //             button: "Месяц",
    //             price: "33 900 ₽",
    //             href: "SpaceinWidget.run('79b6428e-99ed-47bc-8777-0f7f58e135fc');",
    //             isPay: false,
    //             list: [
    //                 { title: "Свободное рабочее место" },
    //                 { title: "Вода, чай и кофе" },
    //                 { title: "Принтер и сканер" },
    //                 { title: "Кухня и душевые" },
    //                 { title: "Доступ 24/7" },
    //                 { title: "5 часов любой переговорной комнаты" },
    //                 { title: "30% скидка на аренду переговорных комнат" },
    //             ],
    //             hint: "",
    //             orderBtnText: "Black - Гость (месяц)",
    //         },
    //     ]
    // },
    {
        title: "Резидент",
        label: "",
        categories: [
            {
                button: "1 месяц",
                price: "30 000 ₽",
                priceBeforeSale: "42 600 ₽",
                label: "АКЦИЯ",
                href: "SpaceinWidget.run('4aec3ed6-1105-4771-bc3b-1206b53fba4e');",
                isPay: false,
                list: [
                    { title: "Фиксированное рабочее место" },
                    { title: "Вода, чай и кофе" },
                    { title: "Принтер и сканер" },
                    { title: "Кухня и душевые" },
                    { title: "Доступ 24/7" },
                    { title: "10 часов любой переговорной комнаты" },
                    { title: "50% скидка на аренду переговорных" },
                ],
                hint: "",
                orderBtnText: "Black - Резидент (1 месяц)",
            },
            // {
            //     button: "3 месяца",
            //     price: "120 800 ₽",
            //     href: "SpaceinWidget.run('95ef20cf-3b7b-46d9-9835-dd725e5b3a98');",
            //     isPay: false,
            //     list: [
            //         { title: "Фиксированное рабочее место" },
            //         { title: "Вода, чай и кофе" },
            //         { title: "Принтер и сканер" },
            //         { title: "Кухня и душевые" },
            //         { title: "Доступ 24/7" },
            //         { title: "10 часов любой переговорной комнаты" },
            //         { title: "50% скидка на аренду переговорных" },
            //     ],
            //     hint: "",
            //     orderBtnText: "Black - Резидент (3 месяца)",
            // },
        ]
    },
];

export const simonovTarif = [
    {
        title: "126 рабочих мест",
        label: "",
        categories: [
            {
                button: "126 рабочих мест",
                price: "5 460 000 ₽/мес",
                href: "",
                isPay: false,
                list: [
                    { title: "Свободное рабочее место" },
                    { title: "Вода, чай и кофе" },
                    { title: "Принтер и сканер" },
                    { title: "Кухня, лаунж-зоны" },
                    { title: "Доступ 24/7" },
                    { title: "Отдельный блок с собственной инфраструктурой" }
                ],
                hint: "*Цена без учета НДС",
                orderBtnText: "Simonov - 126 рабочих мест"
            },
        ]
    },
    {
        title: "234 рабочих места",
        label: "",
        categories: [
            {
                button: "234 рабочих места",
                price: "10 140 000 ₽/мес",
                href: "",
                isPay: false,
                list: [
                    { title: "Свободное рабочее место" },
                    { title: "Вода, чай и кофе" },
                    { title: "Принтер и сканер" },
                    { title: "Кухня, лаунж-зоны" },
                    { title: "Доступ 24/7" },
                    { title: "Отдельный блок с собственной инфраструктурой" }
                ],
                hint: "*Цена без учета НДС",
                orderBtnText: "Simonov -  234 рабочих места"
            },
        ]
    },
    {
        title: "360 рабочих мест",
        label: "",
        categories: [
            {
                button: "360 рабочих мест",
                price: "15 600 000 ₽/мес",
                href: "",
                isPay: false,
                list: [
                    { title: "Свободное рабочее место" },
                    { title: "Вода, чай и кофе" },
                    { title: "Принтер и сканер" },
                    { title: "Кухня, лаунж-зоны" },
                    { title: "Доступ 24/7" },
                    { title: "Вся площадь целиком" }
                ],
                hint: "*Цена без учета НДС",
                orderBtnText: "Simonov -  360 рабочих мест"
            },
        ]
    },
];